...
<head>
	...
	$(document).ready(function() {
		...
		//기술자게시판을 눌렀을때
		$('#a_check_engineerlist').click(function() {
			getBoardListByAdmin(1, "E");
		});
		
		//의뢰인게시판을 눌렀을때
		$('#a_check_clientlist').click(function() {
			getBoardListByAdmin(1, "C");
		});
	});
	
	function boardAdminPaging(nowPage, board_type) {
		getBoardListByAdmin(nowPage, board_type);
	}
	
	//리스트 가져오기
	function getBoardListByAdmin(nowPage, board_type) {
		...
		$.ajax({
			...
			success : function(data) {
				$.each(data.searchList, function(index, boardObj) { // each로 모든 데이터 가져와서 items 배열에 넣고
					//<span class="label label-default"><i class="fa fa-tag f-10 p-r-5 c-gray-light"></i> jquery</span>
					inHTML += "<tr>";
					inHTML += "<td class=\"\">";
					inHTML += "	<input type=\"checkbox\" class=\"boardIds\" id=\"boardIds\" name=\"boardIds\" value=\""+boardObj.board_srl+"\" />";
					inHTML += "</td>";
					inHTML += "<td class=\"\">";
					...
					inHTML += "</td>";
					inHTML += "<td class=\"\">";
					inHTML += "" + boardObj.service_cost + "";
					inHTML += "</td>";
					inHTML += "<td>"+boardObj.service_time_start+"<br/> ~ "+boardObj.service_time_end+"</td>";
					inHTML += "<td>"+boardObj.location+"</td>";
		            inHTML += "<td>"+boardObj.visitcount+"</td>";
					/* inHTML += "<td class=\"\">"; */
					/* inHTML += "<a href=\"${pageContext.request.contextPath}/member/myinfo?action=modifyAdmin&modify_id=" + boardObj.user_id + "\"target=\"_blank\">조회/수정</a>"; */
					/* inHTML += "</td>"; */
					inHTML += "</tr>";
	...
	$(document).ready(function() {
		...
		$('#toggle_check_all').click(function() {
			...
			$(".boardIds").prop("checked", isChecked);
		...
		$("#checkedBoardDeleteBtn").click(function() {
			...
			$(".boardIds").each(function(index, data) {
				if (data.checked) {
			...
			if (!isChecked) {
				//체크
				popLayerMsg("삭제할 게시글을 선택하세요.");
				return;
			}
			
			var checkArray = new Array();
			var inValidDeleteId = false;
			$("input[name='boardIds']:checked").each(function(i) {
			...
			var params = $('#adminBoardListForm').serialize();
			//var params = {"checkArray": checkboxValues };
			//alert(params);
			//alert(checkArray);
			if (confirm("정말 삭제하시겠습니까?")) {
				var url = "${pageContext.request.contextPath}/admin/boards";
				//alert(url);
				$.ajax({
					url : url,
					type : 'delete',
