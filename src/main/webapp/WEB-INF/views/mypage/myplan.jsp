					...
					<div class="col-xs-3">
						<div class="row margin-top-10">
							...
							<div class="col-xs-6 text-right font-size-h6">
								<a href="${pageContext.request.contextPath}/@dmacoder">완성하러가기&nbsp;<i class="fa fa-angle-right"></i></a>
							</div>
						</div>
													...
													<div id="satisfactionInformation" class="dashboard-satisfaction" style="display: none">
														...
														<div class="dashboard-satisfaction-body color-white font-size-h6">
															...
															<div class="position-relative margin-top-5">
																<span>전체</span> <span class="pull-right left-right">0</span>
															</div>

															<div class="position-relative margin-top-5">
																<span>긍정 (&nbsp;>=&nbsp;2.5)</span> <span class="pull-right left-right">0</span>
															</div>

															<div class="position-relative margin-top-5">
																<span>부정 (&nbsp;<&nbsp;2.5)</span> <span class="pull-right left-right">0</span>
															</div>

															<div class="position-relative margin-top-5">
																<span>평가없음</span> <span class="pull-right left-right">0</span>
															</div>
														</div>
													</div>
												</div>
										...
										<div class="row">
											...
											<div class="col-xs-3 text-center border-right">
												...
												<a class="plain" href="${pageContext.request.contextPath}/my_profits">
													<h4 class="header-margin-none margin-top-5">
														<b><i class="fa fa-krw" aria-hidden="true"></i>&nbsp;0</b>
													</h4>
												</a>
											</div>
											<div class="col-xs-3 text-center border-right">
						...
						<div class="row">
							<div class="col-xs-12">
								<ul class="list-inline tab-menu-underline" role="tablist">
									<li id="todoTabLi" role="presentation" class="position-relative dashboardTab  active  "><a href="#todoList" class="todo-header plain font-color-lighter position-relative font-size-h5 NGR" aria-controls="todo" role="tab" data-toggle="tab" onclick="changeTab('todo')">할&nbsp;일&nbsp;<span id="todoCount" class="dashboardAlarm menu-count font-size-h6 border-round">0</span></a></li>

									<li id="alarmTabLi" role="presentation" class="position-relative dashboardTab "><a href="#alarmList" class="plain font-color-lighter font-size-h5 NGR" aria-controls="alarm" role="tab" data-toggle="tab" onclick="changeTab('alarm')">알림&nbsp;<span id="alarmCount" class="dashboardAlarm menu-count font-size-h6 border-round" v-show="unreadCnt > 0" style="display: none">{{ unreadCnt }}</span></a></li>
								</ul>
							</div>
						</div>
							...
							<div id="todoList" role="tabpanel" class="tab-pane fade  in active ">
								<div class="margin-top-50 text-center" v-if="todos.length == 0 && !unreadMessage">
									<div class="position-relative">
										...
										<div class="margin-top-15 font-color-lighter font-size-h4">할 일을 모두 완료했습니다.</div>
									</div>
								</div>
										...
										<li v-if="unreadMessage" class="todos list-group-item">
											<div class="row">
												<a class="plain" href="{{ unreadMessage.extra_data.url }}">
													...
													<div class="col-xs-7 margin-top-10">{{ unreadMessage.message }}</div>
												</a>
												...
												<a class="plain" href="{{ todo.extra_data.url }}">
													...
													<div class="col-xs-7" :class="{'margin-top-10': todo.extra_data.partner == null}">
														...
														<h6 class="header-margin-none font-color-lighter">
															{{ todo.extra_data.partner }}<span v-if="todo.extra_data.expected_completed_at">&nbsp;|&nbsp;발송 예정일 : {{ todo.extra_data.expected_completed_at }}</span>
														</h6>
													</div>
												</a>
												<div id="todo_btn_{{ todo.id }}" class="col-xs-4 todo-list-btn-group todoBtn text-right padding-right-30" style="display: none;">
													<a class="btn btn-primary" href="{{ todo.extra_data.url }}" role="button">{{ todo.extra_data.title }}</a> <a class="btn btn-default" href="javascript:void(0);" role="button" @click="dismiss(todo, $index)"><b><span class="fa fa-check" style="margin-top: 3px;"></span></b></a>
												</div>
											</div>
										</li>
										<li class="list-group-item text-center" :class="{'hidden': isLastTodoPage}"><a href="javascript:void(0);" @click="getTodo()">더 보기</a></li>
									</ul>
								</div>
							</div>
								...
								<div v-if="notifications.length == 0 && gig_request_id == null" class="text-center margin-top-50" style="display: none">
									<div class="position-relative">
										<div class="position-relative">
											...
											<div class="margin-top-15 font-color-lighter font-size-h4">알림 내역이 없습니다.</div>
										</div>
									</div>
								</div>
									...
									<ul class="list-group panel-list-group-only">
										<li class="list-group-item dashboard-list-group-item" v-for="notification in notifications" :class="{'bg-color-unread': notification.unread == 1}">
											<div class="row" v-if="notification.provider == 'ORDER'">
												<div class="col-xs-12">
													<a class="plain" href="${pageContext.request.contextPath}/track/{{ notification.OID }}">
														<div class="row">
															<div class="col-xs-1 padding-right-0 text-center">
															...
															<div class="col-xs-10" :class="{'margin-top-10': todo.extra_data.partner == null}">
																...
																<h6 class="header-margin-none font-color-lighter">
																	{{ notification.time }}&nbsp;ㅣ&nbsp; {{ notification.order.gig_for_noti.category_name }}&nbsp;ㅣ&nbsp; <img src="/img/dashboard/setting.png" style="width: 11px;" v-bind:src="notification.order_notification_type.icon" data-toggle="tooltip" title="{{ notification.order_notification_type.title }}" data-placement="right" />
																</h6>
															</div>
										...
										<li class="list-group-item text-center" :class="{'hidden': is_lastPage}"><a id="notificationsLoadMoreDiv" href="javascript:void(0);" @click="getNotifications('loadMore')">더 보기</a></li>
									</ul>
								</div>
							</div>
